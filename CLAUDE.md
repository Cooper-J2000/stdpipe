...
- `photometry.py` - **Object detection and basic photometry**. Main functions:
  - `get_objects_sep()` - fast SEP-based detection (compiled Python). Supports position-dependent auto-FWHM via `fwhm_spatial_order` (applied per source for aperture/bkgann/optimal PSF width; only the scalar median summary is stored in `obj.meta['fwhm_phot']` so tables stay serializable). Also accepts a callable `fwhm` (e.g. `FWHMMap`) directly.
  ...
  - `estimate_fwhm_from_objects()` - robust mode-based FWHM from a detection table. Scalar by default; `spatial_order >= 1` returns a `FWHMMap` callable (2-D polynomial `FWHM(x, y)`) with transparent scalar fallback on low candidate counts. The spatial fit reuses the scalar path's anti-contamination guard (candidates windowed to 0.5-2x the guarded scalar estimate before the sigma-clipped fit).
